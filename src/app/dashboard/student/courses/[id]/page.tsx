...
export default function StudentCourseViewerPage() {
  ...
  const handleSubmitQuiz = () => {
    ...
    for (const q of activeQuiz.questions) {
      ...
      if (q.type === "MATCHING") {
        // Points awarded per correct pair
        q.options.forEach((opt: any) => {
          total += opt.points || 0
          const studentChoice = ans ? ans[opt.id] : null
          if (studentChoice === opt.id) { // In matching, the 'id' of the option is matched to itself if correct
            earned += opt.points || 0
          }
        })
      } else {
      ...
      return (
        <div className="fixed inset-0 z-[100] bg-gradient-to-br from-slate-900 to-indigo-950 flex flex-col items-center justify-center text-white p-6">
          <div className="max-w-lg w-full text-center space-y-8">
            ...
            <div>
              <p className="text-[11px] font-bold uppercase tracking-[0.3em] text-white/40 mb-3">{passed ? "Congratulations!" : "Keep Practicing"}</p>
